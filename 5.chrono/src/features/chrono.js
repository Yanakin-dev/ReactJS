...
export const chrono = createSlice({
    ...
    reducers: {
        updateChronoValues: (state, action) => {
            const chosenState = state[action.payload.type]; // Session or Pause 
            ...
            if (action.payload.type === "session") {
                ...
                else chosenState.value = chosenState.value + action.payload.value;
            }
            else  if (action.payload.type === "pause") chosenState.value = chosenState.value + action.payload.value;
        },
        tick: (state, action) => {
